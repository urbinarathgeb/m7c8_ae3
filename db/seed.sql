...
VALUES ('create_dimension', 'Crear nuevas dimensiones'),
...
-- SELECT d.id, sc.id, 374, 2.18, '2026-04-27', 'disponible'
-- FROM dimensions d
--          CROSS JOIN stack_configurations sc
-- WHERE d.thickness = 18
--   AND d.width = 90
--   AND d.length = 3600
--   AND sc.width = 11
--   AND sc.height = 34;
...
-- VALUES
INSERT INTO inventory_packages (dimension_id, stack_config_id, unit_count, cubic_meters, production_date, status)
VALUES ((SELECT id FROM dimensions WHERE thickness = 18 AND width = 90 AND length = 3600),
        (SELECT id FROM stack_configurations WHERE width = 11 AND height = 34),
        374, 2.18, '2026-04-27', 'disponible');
